...
ocidURLComponents's setPath:("/arm-manifests/win/AcrobatDCManifest3.msi")
...
##############################
#マニフェスト読み込み
set ocidExtractDirPathURL to ocidSaveDirPathURL's URLByAppendingPathComponent:("AcrobatDCManifest3") isDirectory:(true)
#パス
...
set ocidAcrobatArray to ocidPredicatedArray's sortedArrayUsingSelector:("localizedStandardCompare:")

##############################
#出力用テキスト
set ocidOutPutstring to refMe's NSMutableString's alloc()'s initWithCapacity:(0)
ocidOutPutstring's appendString:("Windows版 Acrobat32bit版の最新パッチURL\nMSI：インストーラー\nMSP：アップデーターパッチ\nMUI：マルチリンガル対応版\nincr：差分のみアップデート版（軽量）\n\n")
ocidOutPutstring's appendString:("----+----1----+----2----+-----3----+----4----+----5----+----6----+----7\n\n")
##############################
#取り出してテキストに
repeat with itemPredicatedArray in ocidPredicatedArray
