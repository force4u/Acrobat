...
ocidOutPutstring's appendString:("Windows版 Reader X64の新規インストーラーURL\n一部のURLはリンク切れになっています\n上部が古いバージョン下部が新しいバージョンです\n")
...
repeat with itemIntNo from (numCntArray - 1) to 0 by -1
	...
	set strURI to ocidURI as text
	#	if strURI contains "/AcrobatDC/2" then
	if strURI contains "/AcrobatDC/" then
		#タイトル
		...
		set ocidLocalizedTitle to (ocidLocalized's stringByReplacingOccurrencesOfString:("Update") withString:("Installer"))
		(ocidOutPutstring's appendString:(ocidLocalizedTitle as text))
		(ocidOutPutstring's appendString:("\n"))
		
		set ocidDownURL to (ocidURI's stringByReplacingOccurrencesOfString:("Upd") withString:(""))
		set ocidDownURL to (ocidDownURL's stringByReplacingOccurrencesOfString:(".msp") withString:("_ja_JP.exe"))
		set ocidDownURL to (ocidDownURL's stringByReplacingOccurrencesOfString:("_MUI_ja_JP.exe") withString:("_MUI.exe"))
		if (ocidDownURL as text) contains "_ja_JP" then
			set ocidDownURL to (ocidDownURL's stringByReplacingOccurrencesOfString:("ardownload3") withString:("ardownload2"))
			set ocidDownURL2 to (ocidDownURL's stringByReplacingOccurrencesOfString:("AcroRdrDC") withString:("AcroRdrDCx64"))
					(ocidOutPutstring's appendString:(ocidDownURL2))
		(ocidOutPutstring's appendString:("\n\n"))
			
		end if
		(ocidOutPutstring's appendString:(ocidDownURL))
		(ocidOutPutstring's appendString:("\n\n"))
