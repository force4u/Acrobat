...
ocidURLComponents's setPath:("/arm-manifests/win/SCUP/AcrobatCatalog-DC.cab")
...
##############################
#マニフェスト読み込み
set ocidExtractDirPathURL to ocidSaveDirPathURL's URLByAppendingPathComponent:("AcrobatCatalog-DC") isDirectory:(true)
#XMLパス
set ocidXmlFilePathURL to ocidExtractDirPathURL's URLByAppendingPathComponent:("Acrobat_Catalog.xml") isDirectory:(false)
##############################
#NSDATAに読み込み
set ocidOption to (refMe's NSDataReadingMappedIfSafe)
...
ocidOutPutstring's appendString:("Windows版 Acrobatの最新パッチURL\n\n")
...
repeat with itemIntNo from (numCntArray - 1) to 0 by -1
	...
	set strLocalized to ocidLocalized as text
	if strLocalized contains "x64" then
