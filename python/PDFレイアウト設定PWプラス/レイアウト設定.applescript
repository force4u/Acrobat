...
【重要】
python3 
XcodeがインストールされていればOKです
https://apps.apple.com/jp/app/xcode/id497799835?mt=12
pip pymupdfが必要です
自動でインストール　アップデートします
不要な方は実行しないでください

許可設定はAcrobatやPDFkitの設定と必ずしも一致しません
設定後は設定後の内容を確認してください

20250212 v1  初回リリース　テスト版
20250213 v1.1  許可番号の計算方法修正 pyファイル名修正
20250213 v1.1.1 setPdfLayout.pyで上書きしないようにした
20250214 v1.1.2 レイアウトの呼称をAcrobatの呼称に変更
20250216 v1.1.3 pyファイルの実行アクセス権処理を追加した
*)
----+----1----+----2----+-----3----+----4----+----5----+----6----+----7
...
if numVerNo ≥ 15 then
	ocidPrefDict's setValue:("/System/Library/Desktop Pictures/Solid Colors/Space Gray.png") forKey:("bannerimage")
else if numVerNo ≥ 14 then
	ocidPrefDict's setValue:("/System/Library/Desktop Pictures/Solid Colors/Turquoise Green.png") forKey:("bannerimage")
else if numVerNo ≥ 13 then
	ocidPrefDict's setValue:("/System/Library/Desktop Pictures/Solid Colors/Yellow.png") forKey:("bannerimage")
else if numVerNo ≥ 12 then
	ocidPrefDict's setValue:("/System/Library/Desktop Pictures/Solid Colors/Silver.png") forKey:("bannerimage")
else
	#【K】エラーアラート
...
#Windowの左上にクローズボタン拡大ボタンを出すか
ocidPrefDict's setValue:(true) forKey:("windowbuttons")
#背景をボカすか？
ocidPrefDict's setValue:(false) forKey:("blurscreen")
#リサイズ
ocidPrefDict's setValue:(false) forKey:("resizable")
#設定【position】
#[topleft | left | bottomleft | top | center/centre | bottom | topright | right | bottomright]
ocidPrefDict's setValue:("top") forKey:("position")
#設定【appearance】　light　dark
ocidPrefDict's setValue:("light") forKey:("appearance")
#設定コマンドログファイル
#COMMANDFILE_PATHは自動付与されるのでこのままにしてください
...
ocidPrefDict's setValue:(620) forKey:("height")
#設定バナーサイズ
ocidPrefDict's setValue:("72") forKey:("bannerheight")
ocidPrefDict's setValue:("PDF開き方レイアウト設定") forKey:("bannertitle")
#設定【title】
ocidPrefDict's setValue:("PDF開き方レイアウト設定") forKey:("title")
#設定【titlefont】
ocidPrefDict's setValue:("name=HiraMaruProN-W4, colour=#000000,weight=bold,size=36,shadow=1") forKey:("titlefont")
#設定【subtitle】※通知用なので使われない
ocidPrefDict's setValue:("インストール処理") forKey:("subtitle")
#設定【message】
ocidPrefDict's setValue:("設定したら実行してください") forKey:("message")
#設定【messageposition】
...
#設定【helpmessage】
ocidPrefDict's setValue:("# 留意事項<br>許可設定はAcrobatやPDFkitの設定と必ずしも一致しません<br>設定値はあくまでもpymupdfの値です<br>設定後<br>設定の内容を確認してください<br>(https://pymupdf.readthedocs.io/ja/latest/vars.html#permissioncodes)") forKey:("helpmessage")
#########
#設定【icon】
...
ocidPrefDict's setValue:("https://quicktimer.cocolog-nifty.com/icefloe/2025/02/post-37f519.html") forKey:("infobuttonaction")
...
#####################
#設定【button1text】
ocidPrefDict's setValue:("実行") forKey:("button1text")
...
set ocidSubArray to refMe's NSMutableArray's alloc()'s init()
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("レイアウトを選んでください") forKey:("title")
ocidSubDict's setValue:("Select01") forKey:("name")
#V1.1.2で変更
#set listLayouKey to {"単ページ", "連続ページ", "見開連続左", "見開連続右", "見開左", "見開右"} as list
set listLayouKey to {"単一ページ", "連続ページ", "連続見開きページ", "連続見開きページ（表紙）", "見開きページ", "見開きページ（表紙）"} as list
ocidSubDict's setValue:(listLayouKey) forKey:("values")
ocidSubDict's setValue:(item 2 of listLayouKey) forKey:("default")
ocidSubDict's setValue:(false) forKey:("required")
ocidSubArray's addObject:(ocidSubDict)

##
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("パネルを選んでください") forKey:("title")
ocidSubDict's setValue:("Select02") forKey:("name")
set listModeKey to {"パネル無", "しおり・ブックマーク", "ページサムネル", "レイヤ", "添付ファイル"} as list
ocidSubDict's setValue:(listModeKey) forKey:("values")
ocidSubDict's setValue:(item 3 of listModeKey) forKey:("default")
ocidSubDict's setValue:(false) forKey:("required")
ocidSubDict's setValue:("list") forKey:("style")
ocidSubArray's addObject:(ocidSubDict)

ocidPrefDict's setValue:(ocidSubArray) forKey:("selectitems")
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("PDFファイル選択") forKey:("title")
ocidSubDict's setValue:("Textfield01") forKey:("name")
ocidSubDict's setValue:("PDFファイルを選んでください") forKey:("prompt")
#ocidSubDict's setValue:("") forKey:("value")
ocidSubDict's setValue:(true) forKey:("fileselect")
ocidSubDict's setValue:("PDF") forKey:("filetype")
ocidSubDict's setValue:(true) forKey:("required")
ocidSubArray's addObject:(ocidSubDict)
#
ocidPrefDict's setValue:(ocidSubArray) forKey:("textfield")
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("【A】開封パスワード設定(パスワードが無いと見れないPDF)") forKey:("label")
ocidSubDict's setValue:("Checkbox01") forKey:("name")
ocidSubDict's setValue:(false) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("【B】所有者パスワード設定(閲覧出来るが変更出来ないPDF)") forKey:("label")
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:低解像度印刷※(A・Bどちらか有効で非許可設定反映します)") forKey:("label")
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:修正・変更※") forKey:("label")
ocidSubDict's setValue:("Checkbox04") forKey:("name")
ocidSubDict's setValue:(true) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:抽出・コピー※") forKey:("label")
ocidSubDict's setValue:("Checkbox05") forKey:("name")
ocidSubDict's setValue:(true) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:注釈・署名※") forKey:("label")
ocidSubDict's setValue:("Checkbox06") forKey:("name")
ocidSubDict's setValue:(true) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:フォーム入力※") forKey:("label")
ocidSubDict's setValue:("Checkbox07") forKey:("name")
ocidSubDict's setValue:(true) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:アクセシビリティ※") forKey:("label")
ocidSubDict's setValue:("Checkbox08") forKey:("name")
ocidSubDict's setValue:(true) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:再構成・属性変更※") forKey:("label")
ocidSubDict's setValue:("Checkbox09") forKey:("name")
ocidSubDict's setValue:(true) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
...
#
set ocidSubDict to refMe's NSMutableDictionary's alloc()'s init()
ocidSubDict's setValue:("許可:高画質印刷※") forKey:("label")
ocidSubDict's setValue:("Checkbox10") forKey:("name")
ocidSubDict's setValue:(true) forKey:("checked")
ocidSubDict's setValue:(false) forKey:("disabled")
ocidSubArray's addObject:(ocidSubDict)
...
if strResponse is false then
	#message更新
	set strLogText to ("button1: disable\nmessage: python3が利用可能ではありません<br>python3をインストールしてください<br>[詳しくはこちら](https://quicktimer.cocolog-nifty.com/icefloe/2024/09/post-84bace.html)") as text
...
try
	...
	doShShellScript(strCommandText)
	set strCommandText to ("\"" & strPyBinPath & "\" -m pip install --user pymupdf") as text
	doShShellScript(strCommandText)
	set strCommandText to ("\"" & strPyBinPath & "\" -m pip install --user pymupdf-fonts") as text
	doShShellScript(strCommandText)
	set strCommandText to ("\"" & strPyBinPath & "\" -m pip install --upgrade --user pymupdf") as text
	doShShellScript(strCommandText)
	set strCommandText to ("\"" & strPyBinPath & "\" -m pip install --upgrade --user pymupdf-fonts") as text
	doShShellScript(strCommandText)
	set strCommandText to ("\"" & strPyBinPath & "\" -m pip show pymupdf | grep Location") as text
	set strPipPath to doShShellScript(strCommandText) as text
	
	
	#message更新
	set strLogText to ("message: python3: 利用可能: " & strPyBinPath & "<br>pymupdf: 利用可能 :" & strPipPath & "") as text
	set strResponse to doWriteLogFile(strLogText, ocidLogFilePathURL)
	if strResponse is false then
		return "更新失敗"
	end if
on error
	set strLogText to ("button1: disable\nmessage: pymupdfが利用可能ではありません") as text
	set strResponse to doWriteLogFile(strLogText, ocidLogFilePathURL)
	if strResponse is false then
		return "更新失敗"
	end if
...
#infobox更新
set strLogText to ("infobox: {osversion} {osname}<br>{computermodel}<br>Disk: " & strDislLeft & "<br>python: " & strPyBinPath & "\nlistitem: title : Python3インストールチェック, status : success, statustext : OK " & strPyBinPath & "\nbutton1: enable") as text
set strResponse to doWriteLogFile(strLogText, ocidLogFilePathURL)
if strResponse is false then
	return "更新失敗"
end if
...
#戻り値で処理
(*
set recordLayout to {|単ページ|:"SinglePage", |連続ページ|:"OneColumn", |見開連続左|:"TwoColumnLeft", |見開連続右|:"TwoColumnRight", |見開左|:"TwoPageLeft", |見開右|:"TwoPageRight"} as record

set recordMode to {|パネル無|:"UseNone", |しおり・ブックマーク|:"UseOutlines", |ページサムネル|:"UseThumbs", |レイヤ|:"UseOC", |添付ファイル|:"UseAttachments"} as record
*)
...
set ocidScriptDirPath to ocidContainerDirPath's stringByAppendingPathComponent:("Script")
set strPyFileNamePath to ("setPdfLayout.py") as text
set ocidPyFilePath to ocidScriptDirPath's stringByAppendingPathComponent:(strPyFileNamePath)
set strPyFilePath to ocidPyFilePath as text
#アクセス権をつけておく
set appFileManager to refMe's NSFileManager's defaultManager()
...
set listDone to appFileManager's setAttributes:(ocidAttrDict) ofItemAtPath:(ocidPyFilePath) |error|:(reference)


############
#ファイルパス
set strSendFilePath to (ocidResponseDict's valueForKey:("Textfield01")) as text
############
#レイアウト番号
set recordLayout to {|単一ページ|:"0", |連続ページ|:"1", |連続見開きページ|:"2", |連続見開きページ（表紙）|:"3", |見開きページ|:"4", |見開きページ（表紙）|:"5"} as record
set ocidLayoutDict to refMe's NSDictionary's alloc()'s initWithDictionary:(recordLayout)
set strSelect01 to (ocidResponseDict's valueForKey:("Select01")) as text
set strSetLayoutNo to (ocidLayoutDict's valueForKey:(strSelect01)) as text
############
#モード番号 ※３欠番
set recordMode to {|パネル無|:"0", |しおり・ブックマーク|:"1", |ページサムネル|:"2", |レイヤ|:"4", |添付ファイル|:"5"} as record
set ocidModeDict to refMe's NSDictionary's alloc()'s initWithDictionary:(recordMode)
set strSelect02 to (ocidResponseDict's valueForKey:("Select02")) as text
set strSetModeNo to (ocidModeDict's valueForKey:(strSelect02)) as text

############
#パスワード設定番号
set boolCheckbox01 to (ocidResponseDict's valueForKey:("Checkbox01")) as boolean
set boolCheckbox02 to (ocidResponseDict's valueForKey:("Checkbox02")) as boolean

if boolCheckbox01 is true then
	set strOpenPermNo to "0" as text
else if boolCheckbox01 is false and boolCheckbox02 is true then
	set strOpenPermNo to "1" as text
else
	set strOpenPermNo to "2" as text
end if

############
#暗号化番号
set strEncNO to "5" as text

############
#許可番号
set recordPerm to {|低解像度印刷|:4, |修正・変更|:8, |抽出・コピー|:16, |注釈・署名|:32, |フォーム入力|:256, |アクセシビリティ|:512, |再構成・属性変更|:1024, |高画質印刷|:2048} as record

set ocidPermDict to refMe's NSDictionary's alloc()'s initWithDictionary:(recordPerm)
set boolCheckbox03 to (ocidResponseDict's valueForKey:("Checkbox03")) as boolean
set boolCheckbox04 to (ocidResponseDict's valueForKey:("Checkbox04")) as boolean
set boolCheckbox05 to (ocidResponseDict's valueForKey:("Checkbox05")) as boolean
set boolCheckbox06 to (ocidResponseDict's valueForKey:("Checkbox06")) as boolean
set boolCheckbox07 to (ocidResponseDict's valueForKey:("Checkbox07")) as boolean
set boolCheckbox08 to (ocidResponseDict's valueForKey:("Checkbox08")) as boolean
set boolCheckbox09 to (ocidResponseDict's valueForKey:("Checkbox09")) as boolean
set boolCheckbox10 to (ocidResponseDict's valueForKey:("Checkbox10")) as boolean
set numPermNo to 3900 as integer
if boolCheckbox03 is false then
	set numPermNo to numPermNo - 4 as integer
end if
if boolCheckbox04 is false then
	set numPermNo to numPermNo - 8 as integer
end if
if boolCheckbox05 is false then
	set numPermNo to numPermNo - 16 as integer
end if
if boolCheckbox06 is false then
	set numPermNo to numPermNo - 32 as integer
end if
if boolCheckbox07 is false then
	set numPermNo to numPermNo - 256 as integer
end if
if boolCheckbox08 is false then
	set numPermNo to numPermNo - 512 as integer
end if
if boolCheckbox09 is false then
	set numPermNo to numPermNo - 1024 as integer
end if
if boolCheckbox10 is false then
	set numPermNo to numPermNo - 2048 as integer
end if
set strPermNo to numPermNo as text
(*
if numPermNo = 3900 then
	set strPermNo to "0" as text
else
	set strPermNo to numPermNo as text
end if
*)
############
#コマンド整形
set strCommandText to ("\"" & strPyBinPath & "\" \"" & strPyFilePath & "\" \"" & strSendFilePath & "\" " & strSetLayoutNo & " " & strSetModeNo & " " & strOpenPermNo & " " & strEncNO & " " & strPermNo & "") as text
log "\r" & strCommandText & "\r"
...
to doChkInstall(argBundleID)
	...
	if ocidAppPathURL = (missing value) then
		log "インストールはされていますがランチDBが更新されていません再度実行してみてください"
		return false
	end if
	##########################
	#本当にあるか？チェック
...
##########################
# 【M】SH　実行
to doShShellScript(argCommandText)
	set strExec to ("" & argCommandText & "") as text
