...
#########################################上段　左
			if numCntInPage >= (numAllPage):
				if (numRotationULL == 0) | (numRotationULL == 180) :
						objPageUCL = PageObject.create_blank_page(width=numWULL, height=numHULL)
				elif (numRotationULL == 90) | (numRotationULL == 270) :
						objPageUCL = PageObject.create_blank_page(width=numHULL, height=numWULL)
				numCropWUCL = objPageUCL.mediabox.right
				numCropHUCL = objPageUCL.mediabox.top
				numRotationUCL = numRotationULL
				numCropXUCL = 0
				numCropYUCL = 0
			else:
...
#########################################上段　右
			if numCntInPage >= (numAllPage):
				if (numRotationUCL == 0) | (numRotationUCL == 180) :
						objPageUCR = PageObject.create_blank_page(width=numWUCL, height=numHUCL)
				elif (numRotationUCL == 90) | (numRotationUCL == 270) :
						objPageUCR = PageObject.create_blank_page(width=numHUCL, height=numWUCL)
				numCropWUCR = objPageUCR.mediabox.right
				numCropHUCR = objPageUCR.mediabox.top
				numRotationUCR = numRotationUCL
				numCropXUCR = 0
				numCropYUCR = 0
			else:
...
#########################################上段　右 右
			if numCntInPage >= (numAllPage):
				if (numRotationUCR == 0) | (numRotationUCR == 180) :
						objPageURR = PageObject.create_blank_page(width=numWUCR, height=numHUCR)
				elif (numRotationUCR == 90) | (numRotationUCR == 270) :
						objPageURR = PageObject.create_blank_page(width=numHUCR, height=numWUCR)
				numCropWURR = objPageURR.mediabox.right
				numCropHURR = objPageURR.mediabox.top
				numRotationURR = numRotationUCR
				numCropXURR = 0
				numCropYURR = 0
			else:
...
#########################################下段　左　左
			if numCntInPage >= (numAllPage):
				if (numRotationURR == 0) | (numRotationURR == 180) :
						objPageBLL = PageObject.create_blank_page(width=numWURR, height=numHURR)
				elif (numRotationURR == 90) | (numRotationURR == 270) :
						objPageBLL = PageObject.create_blank_page(width=numHURR, height=numWURR)
				numCropWBLL = objPageBLL.mediabox.right
				numCropHBLL = objPageBLL.mediabox.top
				numRotationBLL = numRotationURR
				numCropXBLL = 0
				numCropYBLL = 0
			else:
...
#########################################下段　左　
			if numCntInPage >= (numAllPage):
				if (numRotationBLL == 0) | (numRotationBLL == 180) :
						objPageBCL = PageObject.create_blank_page(width=numWBLL, height=numHBLL)
				elif (numRotationBLL == 90) | (numRotationBLL == 270) :
						objPageBCL = PageObject.create_blank_page(width=numHBLL, height=numWBLL)
				numCropWBCL = objPageBCL.mediabox.right
				numCropHBCL = objPageBCL.mediabox.top
				numRotationBCL = numRotationBLL
				numCropXBCL = 0
				numCropYBCL = 0
			else:
...
#########################################下段　右　
			if numCntInPage >= (numAllPage):
				if (numRotationBCL == 0) | (numRotationBCL == 180) :
						objPageBCR = PageObject.create_blank_page(width=numWBCL, height=numHBCL)
				elif (numRotationBCL == 90) | (numRotationBCL == 270) :
						objPageBCR = PageObject.create_blank_page(width=numHBCL, height=numWBCL)
				numCropWBCR = objPageBCR.mediabox.right
				numCropHBCR = objPageBCR.mediabox.top
				numRotationBCR = numRotationBCL
				numCropXBCR = 0
				numCropYBCR = 0
			else:
...
#########################################下段　右　 右
			if numCntInPage >= (numAllPage):
				if (numRotationBCR == 0) | (numRotationBCR == 180) :
						objPageBRR = PageObject.create_blank_page(width=numWBCR, height=numHBCR)
				elif (numRotationBCR == 90) | (numRotationBCR == 270) :
						objPageBRR = PageObject.create_blank_page(width=numHBCR, height=numWBCR)
				numCropWBRR = objPageBRR.mediabox.right
				numCropHBRR = objPageBRR.mediabox.top
				numRotationBRR = numRotationBCR
				numCropXBRR = 0
				numCropYBRR = 0
			else:
